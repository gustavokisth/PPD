                   ...
                   const uint16_t stride, const uint16_t dim_im_out, q7_t * bufferA, q7_t * Im_out)
{
    ...
    #pragma omp parallel for simd private(i_ch_in, i_x, i_y, k_x, k_y) num_threads(numThreads) //omp paralelo para loop for em SIMD com variáveis privadas
#endif
    for (i_ch_in = 0; i_ch_in < ch_im_in; i_ch_in++)
    {
        //printf("%d, %d\n", i_ch_in, omp_get_num_threads());
        for (i_y = 0; i_y < dim_im_out; i_y++)
        {
            for (i_x = 0; i_x < dim_im_out; i_x++)
            {
